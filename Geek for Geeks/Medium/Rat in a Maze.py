...
class Solution:
    def ratInMaze(self, maze):
        ...
        def backtrack(currentRow, currentColumn, stringTillNow):
            if currentRow >= n or currentRow < 0 or currentColumn >= n or currentColumn < 0:
                return
            
            if maze[currentRow][currentColumn] == 2:
                return #when already visited matrix cell
            
            if maze[currentRow][currentColumn] == 0:
                return #when blocked by given matrix
                        
            if currentRow == (n - 1) and currentColumn == (n - 1):
                ans.append(stringTillNow)
                return
            
            maze[currentRow][currentColumn] = 2
            
            backtrack(currentRow, currentColumn + 1, stringTillNow + "R")
            backtrack(currentRow, currentColumn - 1, stringTillNow + "L")
            backtrack(currentRow + 1, currentColumn, stringTillNow + "D")
            backtrack(currentRow - 1, currentColumn, stringTillNow + "U")
...
"""
This is answered by Shashcode. line 60 - 61 can be return as:
...
initialy i wrote:
return ans.sort()
This returns None, not the sorted list.
